...
set +a
IMAGE_NAME="${DOCKER_IMAGE:-llm-sql-dsv4-iq3xxs-dgx-spark:cuda13}"

if ! docker image inspect "${IMAGE_NAME}" >/dev/null 2>&1; then
  echo "[DeepSeek] required offline image is missing: ${IMAGE_NAME}" >&2
  echo "[DeepSeek] load the offline Docker image bundle or build it while online." >&2
  exit 1
fi
...
"${LLAMACPP_DIR}/offline_proxy.sh" start \
  "${DOCKER_CONTAINER:-llm-sql-dsv4-iq3xxs-dgx-spark}" "${PORT:-8080}" "${PORT:-8080}"
