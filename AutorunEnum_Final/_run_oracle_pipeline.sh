...
# Oracle is both the source and the validation target in this launcher, so fail
# before loading a large model if credentials, connectivity, V$SQL, or plan
# privileges are unavailable.
export REQUIRE_ORACLE_VALIDATION=1
"${AUTORUN_PYTHON}" "${SCRIPT_DIR}/oracle_preflight.py" --mode oracle --validation
...
shutdown_models() {
  ...
  if [[ "${leave_qwen_ui_running}" != "1" ]] && flag_enabled "${AUTO_STOP_MODELS}"; then
    ...
    "${DEEPSEEK_DIR}/WithoutUI/stop.sh" >/dev/null 2>&1 || true
...
echo "[AutorunEnum] flow: Qwen draft -> Hy3 critique -> DeepSeek 0731 DS4 final rewrite -> Hy3 final review -> configured Oracle gates"
